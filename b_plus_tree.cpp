...
bool BPLUSTREE_TYPE::GetValue(const KeyType &key,
  ...
  //找到是哪一页
  B_PLUS_TREE_LEAF_PAGE_TYPE *aim = FindLeafPage(key,false,OpType::READ,transaction);
  if (aim == nullptr) return false;//若未找到则直接返回false
  //找value
  result.resize(1);
  auto result = aim->Lookup(key,result[0],comparator_);
  //unPin buffer pool
  FreePagesInTransaction(false,transaction,aim->GetPageId());
  buffer_pool_manager_->UnpinPage(aim->GetPageId(), false);
...
bool BPLUSTREE_TYPE::InsertIntoLeaf(const KeyType &key, const ValueType &value,
  ...
  ValueType v;
  bool exist = leafPage->Lookup(key,v,comparator_);//表示要插入的页是否存在
  //若存在
  if (exist) {
...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction) {
  //若该节点为根节点且仅有一个剩余孩子 删除该节点并使子节点成为新根节点
  if (node->IsRootPage()) {
  ...
  if (node->GetSize() + node2->GetSize() <= node->GetMaxSize()) {
    ...
    Coalesce(node2,node,parentPage,removeIndex,transaction);//unpin 
